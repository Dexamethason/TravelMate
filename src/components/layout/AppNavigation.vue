...
// Dodajemy listener po pierwszym renderze
onMounted(() => {
...
// Usuwamy listener przy zamykaniu komponentu
onBeforeUnmount(() => {
        ...
        <div class="flex justify-between h-16">
          ...
          <div class="flex-shrink-0 flex items-center">
            <router-link to="/" class="flex items-center" @click="handleLogoClick">
              <div
                class="h-8 w-8 xs:h-10 xs:w-10 bg-green-500 rounded-md flex items-center justify-center logo-icon flex-shrink-0"
              >
                <span class="text-white font-bold text-xs xs:text-base">TM</span>
              </div>
              <span class="ml-1 xs:ml-2 text-sm xs:text-lg font-semibold text-gray-900 logo-text"
                >TravelMate</span
              >
            </router-link>
          </div>
          ...
          <nav class="hidden lg:flex items-center ml-4 lg:ml-8 xl:ml-16 flex-shrink">
            <div class="flex space-x-1 lg:space-x-4 xl:space-x-8">
              <router-link
                to="/planner"
                class="text-gray-700 hover:text-gray-900 px-2 lg:px-3 py-2 text-sm font-medium nav-link"
              >
                Planer
              </router-link>
              <router-link
          ...
          <div class="flex items-center space-x-1 sm:space-x-2 md:space-x-4 flex-shrink-0">
            ...
            <div class="hidden lg:block relative">
              <button
                ...
                class="flex items-center text-sm text-gray-700 hover:text-gray-900 px-2 lg:px-3 py-2 cursor-pointer nav-link"
              >
                <span>PL / PLN</span>
            ...
            <template v-if="isLoggedIn">
              ...
              <button
                @click="toggleLogin"
                class="hidden md:inline-flex items-center px-3 lg:px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-gray-500 hover:bg-gray-600 cursor-pointer"
              >
                Wyloguj
              </button>
              <router-link
            ...
            <template v-else>
              ...
              <div class="hidden md:flex space-x-1 md:space-x-2">
                <button
                  @click="toggleLogin"
                  class="inline-flex items-center px-2 md:px-3 lg:px-4 py-2 border border-transparent text-xs sm:text-sm font-medium rounded-md text-white bg-gray-500 hover:bg-gray-600 cursor-pointer"
                >
                  Zaloguj
                </button>
                <router-link
                  to="/auth/register"
                  @click="handleAuthClick"
                  class="inline-flex items-center px-2 md:px-3 lg:px-4 py-2 border border-transparent text-xs sm:text-sm font-medium rounded-md text-white bg-green-500 hover:bg-green-600 cursor-pointer"
                >
            ...
            <div class="flex items-center lg:hidden ml-1 sm:ml-2">
                <button
                    ...
                    class="inline-flex items-center justify-center p-1 xs:p-2 rounded-md text-gray-400 hover:text-gray-900 hover:bg-gray-100 cursor-pointer">
                    <i 
                        :class="[
                        'fas', 
                        isMobileMenuOpen ? 'fa-times' : 'fa-bars',
                        'text-lg xs:text-xl']" ></i>
                </button>
            </div>
          </div>
        </div>
...
/* Stały podkreślnik dla aktywnej strony */
